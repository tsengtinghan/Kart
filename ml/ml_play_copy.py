import logging, sys
sys.path.append(r"C:\Users\user\Desktop\PAIA")
...
class MLPlay:
    ...
    def decision(self, state: PAIA.State) -> PAIA.Action:
        ...
        if state.event == PAIA.Event.EVENT_NONE:
            ...
            # action = agent.choose_action(stacked_img)
            
            # state = state.observation.images.front.data
            action = agent.replay_buffer["action"][agent.step-1]
            reward = state.observation.progress-reward["progress"][agent.step-1]
            total_reward += reward
            agent.replay_buffer["action"][agent.step] = agent.choose_action(stacked_img)
            agent.replay_buffer["progerss"][agent.step] = state.observation.progress
            ...
            action_array = [
                PAIA.create_action_object(acceleration=True, brake=False, steering=0.0),
                PAIA.create_action_object(acceleration=False, brake=False, steering=0), # 不動
                PAIA.create_action_object(acceleration=True, brake=False, steering=-1.0), #left
                PAIA.create_action_object(acceleration=True, brake=False, steering=1.0) # 往前走，右轉
            ]
